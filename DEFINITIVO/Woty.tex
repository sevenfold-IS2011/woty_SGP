...
Il software è sviluppato sul modello client-server. I client si dividono in desktop e mobile. La parte server è ospitata unicamente presso il servizio di cloud computing scelto dall'azienda (si veda il capitolo \ref{hosting} \virgolette{Hosting}).\\
...
\item \textbf{Pro}:
\begin{itemize}
\item \textbf{Elevata scalabilità}: è possibile aumentare dinamicamente il numero di web \textit{dynos} (thread utilizzabili dall'applicazione per produrre risposte a chiamate REST) e di \textit{worker dynos} (thread in background disponibili all'applicazione) in modo da \virgolette{consumare} solo le risorse necessarie a rispondere alle esigenze dell'applicazione in un dato momento.
\item \textbf{Deploy immediato}: grazie all'interfaccia nativa di Heroku ad applicazioni Ruby on Rails, è possibile effettuare il deploy di un'applicazione (e dei suoi successivi aggiornamenti) in maniera immediata, semplicemente effettuando un \emph{push} al repository Git fornito da Heroku. 
\item \textbf{Affidabilità}: l'infrastruttura server di Heroku garantisce un \textit{uptime} del 99.5\% e ridondanza dei dati in location geograficamente separate, in modo da prevenire qualsiasi rischio di calamità naturale. 
\item \textbf{Indipendenza dall'infrastruttura}: Heroku permette di delegare completamente il lavoro di sistemista server, in modo da poter concentrare le energie del team sul perfezionamento di Woty.
\end{itemize}
\item \textbf{Contro}:
\begin{itemize}
\item \textbf{Scalabilità} delle prestazioni, scalabilità del costo: I costi di Heroku sono dipendenti dalle effettive risorse consumate: è necessario monitorare l'andamento delle richieste in modo da non arrivare a costi eccessivamente elevati.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.47]{images/cap4/analisiDiMercato/infortuniPerSesso2}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{images/cap4/analisiDiMercato/infortuniPerEta2}
...
\begin{itemize}
	\item European Agency for Safety and Health at Work (\textbf{EU-OSHA}) (\url{http://osha.europa.eu})
		\begin{itemize}
		...
		\end{itemize}
	\item \textbf{INAIL} (\url{http://www.ispesl.it/ew/ec2012/})
	\item \textbf{PMI Servizi} (\url{http://www.pmiservizi.it/corsi})
		\begin{itemize}
			\item Quotidiano Sicurezza (\url{http://itunes.apple.com/it/app/quotidiano-sicurezza/id419112770?mt=8})
		\end{itemize}
	\item \textbf{ANFOS} (Associazione Nazionale Formatori della Sicurezza sul Lavoro) (\url{http://www.anfos.it})
\end{itemize}
...
\underline{\textbf{costo stimato}}: \EUR 900.
...
\underline{\textbf{costo stimato}}: \EUR 600.
...
\begin{itemize}
	...
	\item \textbf{Corso per formatori di sicurezza:} \EUR \ 250.
...
\begin{itemize}
	\item \textbf{Corso aggiornamento formatori di sicurezza:} \EUR \ 250.
\end{itemize}

\subsubsection{Costi variabili}

Vengono listate le stime dei costi del servizio di cloud computing Heroku calcolate secondo la fascia di utenza minima di un'azienda (ditta fino a 20 persone):

\begin{itemize}
	\item \textbf{Costi Infrastruttura Heroku:}
	\begin{itemize}
		\item fino a \textbf{5} clienti: \EUR \ 34.
		\item fino a \textbf{50}: \EUR \ 180. 
		\item fino a \textbf{150}: \EUR \ 344.
		\item fino a \textbf{300}: \EUR \ 650.
	\end{itemize}
\end{itemize}

Con ulteriori maggiorazioni a seconda delle aziende che aderiranno al servizio e della loro fascia di utenze.

\subsubsection{Utenti e guadagni previsti}

Essendo una start-up innovativa e non avendo quindi riscontri nel mercato corrente non si può stimare il numero di potenziali clienti ed è per questo che la commercializzazione del prodotto è ristretta inizialmente solo all'ambito nazionale.
